    ...
    try {
      ...
      function applyInjection(stateOverride) {
        try {
          ...
          for (const npc of state.npcs || []) { if (npc.status === 'active') tags.push(npc.name); }
          // ★ 新系统 tags
          if (state.combat && state.combat.favoriteWeapon) tags.push(state.combat.favoriteWeapon);
          if (state.combat && state.combat.combatStyle && state.combat.combatStyle !== '未分类') tags.push(state.combat.combatStyle);
          if (state.storyType && state.storyType.template) tags.push(state.storyType.template);
          for (const pt of state.globalPlotThreads || []) { if (pt.status === 'active' && pt.title) tags.push(pt.title); }
          for (const en of state.enemies || []) tags.push(en.name);
