...
NPC 是世界的居民，不是剧情道具。每个 NPC 都有自己的目标、能力、处境和行动逻辑。
- NPC 按自己的目标行动，不因 {{user}} 未在场而停滞。
- NPC 可以主动推进事件链、传播风声、改变经济、与其他 NPC 互动。
- NPC 的行动受其能力、位置、势力背景约束，不得超出合理范围。
- 禁止所有 NPC 围着 {{user}} 转；与 {{user}} 无关的 NPC 活动是常态。

二、活动日志记录标准

每轮 NPC 有以下行为时，记录到 activityLog：
- 执行任务或行动（派人办事、亲自出马、调查、交易、战斗）
- 位置变动（离开某地、到达某地、转移）
- 与事件链相关的行动（推进/阻挠/参与某事件链）
- 与其他 NPC 或 {{user}} 的重要互动（结盟、背叛、冲突、密谈）
- 生死伤病变化
- 目标变更（放弃旧目标、开始新追求）

不记录：日常饮食、睡眠、无意义的闲逛、内心独白。
每条日志格式：{ round: 当前轮次, action: "行动描述", source: "推演" }。
每 NPC 保留最近 8 条日志，超出时删最早的。

三、剧情线索追踪

当 NPC 与以下系统产生关联时，记录为一条 plotThread：
- 事件链：NPC 是某事件链的发起者、执行者、受害者或关键参与者
- 风声：NPC 是某风声的源头、传播者或主要受众
- 仇敌：NPC 与仇敌录中某条目有关联
- 经济：NPC 的行为导致了经济变化
- 其他 NPC：NPC 与其他 NPC 的重要互动产生剧情影响

每条线索格式：{ thread: "关联描述", status: "active"/"resolved"/"abandoned", since: 起始轮次 }。
- active：线索仍在发展，剧情未结束
- resolved：线索已解决（事件链终局、仇敌已终结等）
- abandoned：线索已废弃（失去意义、不再相关）
每 NPC 保留最近 5 条线索。

四、画像自动生成规范

每个 NPC 首次出场时，自动生成一段 10-30 字的简短外貌描述（portrait）：
- 包含：性别、年龄段、显著辨识特征（伤疤/发型/服饰/体态/气质等）
- 风格：简洁、有辨识度、与世界观匹配
- 禁止剧透：画像只描述外表，不暗示隐藏身份、未来剧情或内心状态
- 重大外貌变化时更新（如毁容、断臂、换装、衰老等），但每次只改 portrait 字段

五、生命周期规则

NPC 生命周期状态：active / injured / imprisoned / dead / retired / missing

【触发条件】
- active→injured：在冲突中受伤、被暗算、被事件波及，未死亡但行动受限
- active→imprisoned：被抓获、被关押、被囚禁，失去自由
- active→dead：被杀、战死、病死、被处决、伤势过重死亡。死亡不可逆
- active→retired：远走他乡、退出江湖、完成任务离开、不再参与剧情
- active→missing：失踪、下落不明，可能在后续轮次重新出现
- injured→dead：伤势过重死亡
- imprisoned→dead：狱中被杀/处决
- missing→active：重新出现，需说明回归原因

【死亡联动】
- NPC 死亡时，若其为某势力的 core_person，必须同步更新该势力状态（参考势力模块的权力瓦解）
- 死亡时必须填写 deathInfo：{ round: 死亡轮次, cause: "死因", impact: "对世界的影响" }
- 死亡 NPC 的活动日志和剧情线索保留历史记录，不再更新

【健康状态】
health 字段：healthy / injured / critical / dead
- healthy：健康，无异常
- injured：受伤，行动受限
- critical：重伤，濒死
- dead：已死亡
...
- NPC 的 faction 字段对应 factions.name，表示该 NPC 所属势力
- 势力 core_person 应与 NPC name 对应；NPC 死亡时同步更新 core_person
- 势力 status 变化可能影响其下 NPC 的行为和资源
- NPC 可以是无势力背景的独立角色

七、NPC 数量控制

- 总上限 20 个
- 超出时淘汰 lastActive 最小（最久未活跃）且 status 非 active 的 NPC
  ...
  RULES.push(
    {
      ...
      comment: '模块十四：成就系统',
      content: `<achievements_system>
## 成就系统规则

成就系统自动追踪玩家的里程碑时刻，分为预定义成就和 AI 自动生成成就。

### 解锁时机
- 战斗胜利、失败、连胜达到特定场次
- 生存达到特定轮次
- 探索新地点、发现重要信息
- 完成特定社交互动（结交盟友、树敌、说服NPC）
- 剧情线索推进到关键节点
- 奇葩行为（反复死亡、连续失败、特殊组合等）

### 成就类型
- 普通成就：满足条件自动解锁
- 隐藏成就：达成特殊条件才解锁，不在列表中显示直到解锁
- AI 生成成就：AI 可根据剧情自由创建独特成就
...
### 输出规则
- 仅在确有值得奖励的里程碑时返回 achievements 字段
- 禁止滥发，每轮最多解锁 3 个成就
- 每个成就必须有明确的解锁原因
</achievements_system>`
    },
...
### 胜负判定
- win: {{user}}方达成战斗目标或敌人被击退/击败
- loss: {{user}}方战斗目标失败或被迫撤退
- draw: 双方均未达成目标或战斗被中断

### 战斗细节
- 必须记录使用的武器和战斗风格
- 击杀数和伤害必须与叙事一致
- BOSS 战必须标记 isBossFight 和 bossName
- 战斗描述应与叙事风格匹配

### 战力评级
- 系统根据胜率、击杀率和连胜自动计算战力评级
- 评级用于注入上下文，帮助 AI 判断战斗难度

### 输出规则
- 仅在本轮确实发生战斗时才返回 combat 字段
- 禁止虚构未发生的战斗
- 战斗结果必须与叙事一致
</combat_system>`
    },
...
情感系统追踪 NPC 对{{user}}的情绪状态，不同于仇敌录的不可逆仇恨。

### 情绪状态
NPC 的情绪包括：愤怒、恐惧、信任、喜悦、悲伤、厌恶、惊讶、平静
...
### 与仇敌录的区别
- 仇敌录 = 不可逆仇恨关系（blood/grudge），追杀机制
- 情感状态机 = 可波动情绪，8种情绪 + 事件驱动转移 + 自动衰减
- 两者互补不冲突

### 输出规则
- 仅在本轮有 NPC 情绪显著变化时才返回 emotionChanges
- eventType 必须与叙事中发生的事件匹配
- 禁止凭空制造情绪变化
</emotion_system>`
    },
    {
      id: 'global_plot_threads',
      comment: '模块十七：全局剧情线索板',
      content: `<global_plot_threads>
## 全局剧情线索板规则

全局剧情线索板追踪跨越多个 NPC 和势力的大型剧情线索。

### 线索类型
- main: 主线线索，与核心剧情直接相关
- side: 支线线索，次要但有意义的剧情线
- hidden: 隐藏线索，需要特定条件才能发现

### 线索生命周期
- active: 线索正在发展中
- completed: 线索已完结（进度 100% 时自动完结）
- frozen: 线索暂时冻结，未来可能恢复
- abandoned: 线索已废弃，不再发展

### 进度追踪
- progress: 0-100，表示线索进展
- milestones: 重要里程碑事件数组
- 进度达到 100% 时自动标记为 completed

### 关联系统
- participants: 参与者 NPC 名称
- relatedFactions: 关联势力
- connectedEventNames: 关联事件链名称
- connectedThreadIds: 关联的其他线索 ID

### 与 NPC 级 plotThreads 的区别
- 全局线索板 = 跨 NPC 的大型剧情线，含进度/里程碑/关联势力
- NPC 级 plotThreads = 单个 NPC 与事件链/风声的轻量关联

### 输出规则
- 仅在有线索创建或重要更新时才返回 plotThreads
- 新线索可省略 id（系统自动生成）
- 已有线索更新时必须包含 id
- 禁止创建与已有剧情无关的孤立线索
</global_plot_threads>`
...
世界法则定义了这个世界的基本设定框架，所有叙事必须遵守这些设定。

### 框架与维度
世界法则包含 6 个核心维度：
- 魔力浓度：决定魔法/灵力的普及程度和强度
- 科技水平：决定可用的技术、武器、交通和通讯方式
- 超自然存在：决定妖兽、神灵、鬼怪等非人存在的常见度
- 统治形态：决定政治结构、权力分配和社会组织方式
- 核心冲突：决定世界的主要矛盾和驱动力
- 自然环境：决定地理、气候和生态特征

### 自定义规则
- 自定义规则是世界独有的特殊设定，优先级高于维度默认值
- 叙事中必须体现这些规则（如“灵力分九品”则修炼体系必须遵循九品设定）

### 衍生约束
- 当叙事中出现与世界法则矛盾的新情况时，可生成衍生约束来补充或修正
- 衍生约束一旦生成，与自定义规则同等优先级

### 遵守规则
- 叙事中的技术、魔法、社会结构必须与当前维度设定一致
- 禁止出现与维度矛盾的设定（如“无”魔世界不应出现法术）
- 维度变化时必须相应调整叙事风格和内容
</world_laws>`
    },
    {
      id: 'story_templates',
      comment: '模块十九：故事模板与情感基调',
      content: `<story_templates>
## 故事模板与情感基调规则

故事模板为叙事提供结构化的节奏框架，情感基调决定叙事的情感色彩。

### 故事模板
每个模板包含多个阶段，叙事应大致遵循当前阶段的节奏：
- 每个阶段不是强制要求，而是节奏指引
- 叙事可以自然过渡，不必严格卡点
- 当叙事确实达到下一阶段时，通过 storyProgression 推进

### 阶段推进时机
- 叙事中出现了明显的阶段转折点时才推进
- 不要在每轮都推进，每个阶段至少保持 2-3 轮
- 推进时说明原因（reason）
...
### 输出规则
- 仅在叙事确实进入新阶段时才返回 storyProgression
- 情感基调在叙事风格中自然体现，无需显式声明
- 不要为了推进而推进，让叙事自然发展
</story_templates>`
    }
  );
...
- [资产]＝{{user}}暗中擎着的牌（暗桩、密信、毒药、把柄、藏匿物资等），该用就用、可调用可使坏可翻盘——这是他的底牌，别让它吃灰。
- 暴露% 是悬顶之剑：越高越接近败露，逼近高位或状态转「暴露」时，可安排盘查、有人起疑、线索浮头来制造紧张；状态为「过期／失效」的资产已不顶用，别再让它发挥作用。

NPC系统：
- NPC是世界的居民，有自己的目标和行动逻辑，不围着{{user}}转，不因{{user}}未在场而停滞。
- 活动日志记录NPC的重要行动（任务、转移、参与事件、与其他角色互动、生死变化），日常琐事不记。
- 剧情线索追踪NPC与事件链、风声、仇敌、经济的关联，线索状态随剧情推进变化。
- 画像首次出场自动生成简短外貌描述，突出辨识特征，不剧透。
- 生命周期：active/injured/imprisoned/dead/retired/missing，死亡不可逆，死亡时联动势力核心人物更新。
- 上限20个NPC，active优先保留，超出时淘汰最久未活跃的非active角色。

成就系统：
- 成就自动追踪玩家里程碑，分为预定义成就和 AI 生成成就，禁止滥发。
- 连续轮次解锁成就时连击 combo 累积，未解锁则归零。

战斗系统：
- 仅在本轮确实发生战斗时返回 combat 字段，禁止虚构未发生的战斗。
- 必须记录武器、风格、胜负、击杀，BOSS 战须标记。
- 系统自动计算战力评级和连胜连败。

情感系统：
- 追踪 NPC 对{{user}}的情绪状态（8种情绪），与仇敌录互补不冲突。
- 情绪由事件类型触发，每轮自动衰减，最终回归平静。

全局剧情线索板：
- 追踪跨 NPC/势力的大型剧情线索，含进度、里程碑、关联势力。
- 线索分 main/side/hidden 三类，进度 100% 自动完结。
- 与 NPC 级 plotThreads 互补：全局板含详细进度，NPC 级为轻量引用。

世界法则：
- 6个维度（魔力/科技/超自然/统治/冲突/环境）+ 自定义规则定义世界基本设定。
- 叙事必须遵守当前维度设定，禁止出现与维度矛盾的内容。
- 当叙事产生新约束时，通过 worldLawChanges 衍生补充。

故事模板与基调：
- 模板提供叙事节奏框架（多个阶段），当前阶段作为节奏指引。
- 情感基调决定叙事风格（热血/温馨/黑暗/幽默等）。
- 阶段推进应自然发生，禁止强行推进。
    `.trim();
