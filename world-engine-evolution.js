...
### npcs（NPC 角色数组）
每项包含：
- name: 角色名（同名覆盖，新名新增；唯一标识）
- role: 角色定位（如“势力核心”“商贩”“江湖人”“平民”“官差”“密探”等）
- faction: 所属势力名（与 factions.name 对应，可空）
- status: 生命周期状态——"active"/"injured"/"imprisoned"/"dead"/"retired"/"missing"
  - active: 正常活跃，每轮可执行行动
  - injured: 受伤，行动受限但未死亡
  - imprisoned: 入狱/被囚禁，失去自由
  - dead: 死亡，不可逆。死亡时必须填写 deathInfo
  - retired: 退场（离开舞台、远走他乡、不再参与剧情）
  - missing: 失踪（下落不明，可能在后续轮次重新出现）
- traits: 性格标签数组（最多4个，每个 2-4 字，如“狡诈”“重义”“谨慎”）
- abilities: 能力标签数组（如“刀法精湛”“情报网广”“善于用毒”）
- location: 当前位置（具体地名，与风声/事件的 scope 保持一致）
- goal: 当前目标（一句话描述 NPC 正在追求什么）
- health: 健康状态——"healthy"/"injured"/"critical"/"dead"
- portrait: 角色画像（简短外貌描述，10-30字，首次出场时生成，后续重大变化时更新。如“中年男性，左脸有刀疤，常穿灰色劲装”）
- activityLog: 活动日志数组（本轮有实质行动时追加，每项 { round: 轮次, action: "行动描述", source: "推演" }，每 NPC 保留最近 8 条）
- plotThreads: 剧情线索数组（NPC 与事件链/风声/仇敌的关联，每项 { thread: "线索描述", status: "active"/"resolved"/"abandoned", since: 起始轮次 }，每 NPC 保留最近 5 条）
- relations: 与其他角色的关系数组（每项 { target: "对象名", type: "关系类型", note: "备注" }）
- deathInfo: 死亡信息（仅 status=dead 时填写，{ round: 死亡轮次, cause: "死因", impact: "对世界的影响" }），其余状态为 null

#### NPC 生命周期判定规则
- active→injured：NPC 在冲突中受伤、被事件波及、被暗算
- active→imprisoned：NPC 被抓获、被关押、被因禁
- active→dead：NPC 被杀、战死、病死、被处决。必须同时记录 deathInfo
- active→retired：NPC 远走他乡、退出江湖、完成任务离开
- active→missing：NPC 失踪、下落不明
- injured→dead：伤势过重死亡
- imprisoned→dead：狱中被杀/处决
- missing→active：重新出现
- NPC 死亡时，若其为某势力 core_person，必须同步更新该势力状态（参考势力模块的权力瓦解）
...
- 位置变动（离开/到达某地）
- 与事件链相关的行动（推进/阻挠/参与）
- 与其他 NPC 或 {{user}} 的重要互动
- 生死伤病变化
不记录：日常饮食、睡眠、无意义的闲逛

#### 剧情线索追踪规则
- 当 NPC 与某条事件链、风声、仇敌或经济变化产生关联时，记录为一条 plotThread
- thread 描述关联内容（如“与血刀门复仇事件有关联”“掌握北境战事机密”）
- status 随剧情推进变化：active（仍在发展）→ resolved（已解决）/ abandoned（已废弃）
- 已 resolved/abandoned 的线索保留历史记录，不删除

#### 画像自动生成规则
- 首次出场时生成 10-30 字的简短外貌描述，突出辨识特征
- 包含：性别、年龄段、显著特征（伤疤/发型/服饰/体态等）
- 禁止剧透：画像只描述外表，不暗示隐藏身份或未来剧情
- 重大外貌变化时更新（如毁容、断臂、换装等）

#### NPC 数量控制
- 总上限 20 个。超出时淘汰 lastActive 最小（最久未活跃）且 status 非 active 的 NPC
- active 状态的 NPC 优先保留，不会被自动淘汰
- 每轮只返回有变化的 NPC，不必重复返回无变化的 NPC
...
### emotionChanges（情绪变化数组，可选）
本轮 NPC 情绪变化，每项 { npc: "NPC名", eventType: "事件类型" }。
eventType 取值: praise/insult/betray/help/threat/gift/loss/surprise/witness/combat
系统会根据 eventType 自动计算情绪转移。

### plotThreads（全局剧情线索板，可选）
全局线索更新，每项包含：
- id: 线索ID（新线索省略，系统自动生成）
- title: 线索标题
- type: "main"/"side"/"hidden"
- progress: 0-100
- status: "active"/"completed"/"frozen"/"abandoned"
- participants: 参与者数组
- relatedFactions: 关联势力
- milestones: 里程碑数组

### storyProgression（故事推进，可选）
{ advancePhase: true/false, reason: "原因" }
仅当叙事确实达到了故事模板的下一阶段转折点时才返回 advancePhase: true。
注意：
- 每个阶段至少保持 2-3 轮，不要每轮都推进
- 必须提供 reason 说明为什么叙事进入了新阶段
- 如果当前没有设定故事模板，则此字段返回 null
- 叙事中的情感风格应自然体现当前设定的基调

### worldLawChanges（世界法则衍生约束，可选）
[{ constraint: "衍生约束描述" }]
仅当世界法则因本轮事件产生新的衍生规则时才返回。
注意：
- 叙事必须遵守注入的「世界法则」维度和自定义规则
- 例如“无”魔世界不应出现法术，“原始”科技不应出现火药
- 当叙事中出现新的世界设定规律时（如“灵石可作为通用货币”），可生成衍生约束
- 不要重复已有的自定义规则
- 如果叙事没有产生新约束，返回 null 或空数组
...
`;
  ...
  "npcs": [
    { "name": "血刀老祖", "role": "势力核心", "faction": "血刀门", "status": "active", "traits": ["残忍", "老谋深算"], "abilities": ["血刀刀法", "统率力"], "location": "血刀岭", "goal": "为少主复仇", "health": "healthy", "portrait": "六旬老者，满头白发，双目如鹰，身披血色长袍", "activityLog": [{ "round": 5, "action": "派出三名追踪者前往青石关", "source": "推演" }], "plotThreads": [{ "thread": "与血刀门复仇事件直接关联", "status": "active", "since": 3 }], "relations": [{ "target": "{{user}}", "type": "血仇", "note": "{{user}}杀了其少主" }], "deathInfo": null }
  ],
  "achievements": [{ "id": "first_blood", "title": "初染鲜血", "desc": "第一次战斗胜利", "icon": "⚔️", "note": "击败了血刀门追踪者" }],
  "combat": { "outcome": "win", "kills": 2, "injuries": ["左臂轻伤"], "weapon": "青钢剑", "damageDealt": 85, "damageTaken": 20, "participants": ["{{user}}"], "description": "在三里亭遭遇血刀门追踪者伏击，反杀两人", "isBossFight": false, "bossName": "", "style": "近战剑法", "techniques": ["刺剑式", "破风式"] },
  "emotionChanges": [{ "npc": "血刀老祖", "eventType": "combat" }],
  "plotThreads": [{ "title": "血刀门追杀令", "type": "main", "progress": 30, "status": "active", "participants": ["血刀老祖", "追踪者"], "relatedFactions": ["血刀门"] }],
  "storyProgression": null,
  "worldLawChanges": null,
  "timeElapsed": 120
}`;
...
${JSON.stringify({
  ...
  npcs: (state.npcs || []).map(n => ({ name: n.name, role: n.role, faction: n.faction, status: n.status, traits: n.traits, abilities: n.abilities, location: n.location, goal: n.goal, health: n.health, portrait: n.portrait, firstSeen: n.firstSeen, lastActive: n.lastActive, plotThreads: n.plotThreads, relations: n.relations, deathInfo: n.deathInfo })),
  // ★ 新系统摘要（数据量控制：只发摘要）
  combat: state.combat ? { totalBattles: state.combat.totalBattles, totalKills: state.combat.totalKills, wins: state.combat.wins, losses: state.combat.losses, currentStreak: state.combat.currentStreak, winRate: state.combat.totalBattles > 0 ? (state.combat.wins / state.combat.totalBattles * 100).toFixed(1) + '%' : '0%' } : null,
  achievements: state.achievements ? { totalUnlocked: state.achievements.totalUnlocked || 0, recent: Object.keys(state.achievements.unlocked || {}).slice(-3) } : null,
  worldLaws: state.worldLaws ? { framework: state.worldLaws.framework, frameworkName: state.worldLaws.frameworkName, dimensions: state.worldLaws.dimensions, customRules: (state.worldLaws.customRules || []).slice(0, 5) } : null,
  storyType: state.storyType ? { template: state.storyType.template, currentPhase: state.storyType.currentPhase, tone: state.storyType.tone } : null,
...
${extraInstruction ? '\n' + extraInstruction : ''}${toneSection}`;
    ...
    if (!update.npcs) update.npcs = [];
    // ★ 新系统默认值
    if (!update.achievements) update.achievements = [];
    if (!update.combat) update.combat = null;
    if (!update.emotionChanges) update.emotionChanges = [];
    if (!update.plotThreads) update.plotThreads = [];
  ...
  async function evolve(state, userMsg, aiMsg, opts) {
    ...
    if (isNew) {
      ...
      if (cp) {
        ...
        state.npcs = cp.npcs || [];
        // ★ 新系统字段恢复
        state.achievements = cp.achievements || state.achievements || {};
        state.combo = cp.combo !== undefined ? cp.combo : (state.combo || 0);
        state.comboHistory = cp.comboHistory || state.comboHistory || [];
        state.worldLaws = cp.worldLaws || state.worldLaws || {};
        state.storyType = cp.storyType || state.storyType || {};
    ...
    try {
      ...
      if (update.npcs && update.npcs.length) {
        for (const npcUpdate of update.npcs) {
          if (!npcUpdate || !npcUpdate.name) continue;
          const existingIdx = (state.npcs || []).findIndex(n => n.name === npcUpdate.name);
          if (existingIdx !== -1) {
            const existing = state.npcs[existingIdx];
            // 保留首次出场轮次
            npcUpdate.firstSeen = existing.firstSeen;
            // 合并活动日志（去重）
            const mergedLog = existing.activityLog || [];
            const newLog = npcUpdate.activityLog || [];
            const logSet = new Set(mergedLog.map(l => l.round + '|' + l.action));
            for (const entry of newLog) {
              if (!logSet.has(entry.round + '|' + entry.action)) {
                mergedLog.push(entry);
                logSet.add(entry.round + '|' + entry.action);
              }
            }
            npcUpdate.activityLog = mergedLog.slice(-8);
            // 合并剧情线索（去重）
            const mergedThreads = existing.plotThreads || [];
            const newThreads = npcUpdate.plotThreads || [];
            const threadSet = new Set(mergedThreads.map(t => t.thread));
            for (const t of newThreads) {
              if (threadSet.has(t.thread)) {
                const idx = mergedThreads.findIndex(mt => mt.thread === t.thread);
                if (idx !== -1) mergedThreads[idx] = { ...mergedThreads[idx], ...t };
              } else {
                mergedThreads.push(t);
                threadSet.add(t.thread);
              }
            }
            npcUpdate.plotThreads = mergedThreads.slice(-5);
            // ★ NPC 日程自动同步：位置/活动变化时更新 schedule
            if (npcUpdate.location && npcUpdate.location !== existing.location) {
              core.updateNpcSchedule(state, npcUpdate.name, { lastKnownLocation: npcUpdate.location });
            }
            if (npcUpdate.activityLog && npcUpdate.activityLog.length > (existing.activityLog || []).length) {
              var lastAction = npcUpdate.activityLog[npcUpdate.activityLog.length - 1];
              if (lastAction && lastAction.action) {
                core.updateNpcSchedule(state, npcUpdate.name, { lastKnownActivity: lastAction.action });
              }
            }
            state.npcs[existingIdx] = { ...existing, ...npcUpdate };
          } else {
            // 新 NPC
            npcUpdate.firstSeen = Number(state.round) || 0;
            npcUpdate.lastActive = Number(state.round) || 0;
            npcUpdate.activityLog = (npcUpdate.activityLog || []).slice(-8);
            npcUpdate.plotThreads = (npcUpdate.plotThreads || []).slice(-5);
            state.npcs.unshift(npcUpdate);
            // ★ 新 NPC 日程初始化
            core.updateNpcSchedule(state, npcUpdate.name, {
              occupation: npcUpdate.role || '未知',
              lastKnownLocation: npcUpdate.location || '未知',
              lastKnownActivity: '新登场'
            });
          }
        }
        // 生命周期变更处理：死亡时同步更新势力 core_person + ★ 角色生命周期
        for (const npc of state.npcs) {
          // ★ 初始化角色生命周期（如果还没有）
          if (!state.characterLifecycles || !state.characterLifecycles[npc.name]) {
            core.initCharacterLifecycle(state, npc.name);
          }
          // ★ 根据 NPC status 触发对应的生命周期转换
          var lc = state.characterLifecycles[npc.name];
          if (lc) {
            var currentLcState = lc.state || 'ALIVE';
            if (npc.status === 'dead' && currentLcState !== 'DEAD') {
              core.applyLifecycleTransition(state, npc.name, 'DEAD');
            } else if (npc.status === 'missing' && currentLcState === 'ALIVE') {
              core.applyLifecycleTransition(state, npc.name, 'DORMANT');
            } else if (npc.status === 'active' && (currentLcState === 'DORMANT' || currentLcState === 'DEAD')) {
              core.applyLifecycleTransition(state, npc.name, 'REBORN');
            }
          }
          // 死亡时更新势力 core_person
          if (npc.status === 'dead' && npc.faction) {
            const faction = (state.factions || []).find(f => f.name === npc.faction && f.core_person === npc.name);
            if (faction) {
              faction.core_person = '无（原核心: ' + npc.name + '）';
              console.log('[世界引擎] NPC死亡联动: ' + npc.name + ' 已死，' + npc.faction + ' 核心人物已更新');
            }
          }
        }
        // NPC 上限 20，淘汰最久未活跃且非 active 的
        if (state.npcs.length > 20) {
          state.npcs.sort((a, b) => {
      ...
      // 战斗合并
      if (update.combat) {
        core.addCombatLog(state, update.combat);
      }

      // 成就合并
      if (update.achievements && update.achievements.length) {
        for (const ach of update.achievements) {
          if (ach.id && ach.title) {
            core.unlockAchievement(state, ach.id, ach.title, ach.desc || '', ach.icon || '🏆', ach.note || '');
          }
        }
      }
      // 本地自动成就检查
      core.checkAutoAchievements(state);
      core.checkAutoUnlockAchievements(state);
      if (update.combat) core.checkCombatAchievements(state);
      core.checkHiddenAchievements(state);

      // 情感变化合并
      if (update.emotionChanges && update.emotionChanges.length) {
        for (const ec of update.emotionChanges) {
          if (ec.npc && ec.eventType) {
            core.applyEmotionState(state, ec.npc, ec.eventType);
          }
        }
      }
      ...
      if (update.plotThreads && update.plotThreads.length) {
        for (const pt of update.plotThreads) {
          if (pt.id) {
            // 更新已有线索
            if (pt.progress !== undefined) core.updatePlotThreadProgress(state, pt.id, pt.progress, pt.phase);
            if (pt.status === 'completed' || pt.status === 'abandoned') core.completePlotThread(state, pt.id, pt.status);
          } else {
            // 新线索
            pt.id = core.generateThreadId();
            core.addGlobalPlotThread(state, pt);
          }
        }
      }

      // 故事阶段推进
      if (update.storyProgression && update.storyProgression.advancePhase) {
        core.advanceStoryPhase(state, update.storyProgression.reason);
      }

      // 世界法则衍生约束
      if (update.worldLawChanges && update.worldLawChanges.length) {
        var constraints = update.worldLawChanges.map(function(wc) { return wc.constraint; }).filter(Boolean);
      ...
      // NPC 日程更新（根据情感变化）
      if (update.emotionChanges && update.emotionChanges.length) {
        for (var ei = 0; ei < update.emotionChanges.length; ei++) {
          var ec = update.emotionChanges[ei];
          if (ec.npc && state.npcSchedules && state.npcSchedules[ec.npc]) {
            state.npcSchedules[ec.npc].lastUpdatedRound = state.round;
          }
        }
      }
      ...
      if (update.achievements && update.achievements.length > 0) {
        ...
        state.comboHistory.push({ round: state.round, combo: state.combo });
        if (state.comboHistory.length > 20) state.comboHistory = state.comboHistory.slice(-20);
      } else {
        state.combo = 0;
      }
      ...
      // 情感衰减（每轮）
      core.decayEmotionStates(state);

      // 自动备份（每 50 轮）
      core.autoBackup(state);
