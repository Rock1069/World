...
window.WORLD_ENGINE_INJECT = (function() {
  ...
  function buildContext(worldState, tags) {
    ...
    // NPC 系统：只注入 active 的 NPC，按重要性排序，最多 8 个
    const STATUS_LABEL = { active: '活跃', injured: '受伤', imprisoned: '被囚', dead: '已故', retired: '退场', missing: '失踪' };
    const activeNpcs = (worldState.npcs || []).filter(n => n.status === 'active');
    // 排序：有 faction 优先，有 plotThreads 优先，lastActive 大者优先
    activeNpcs.sort((a, b) => {
      const aScore = (a.faction ? 2 : 0) + ((a.plotThreads || []).length ? 1 : 0);
      const bScore = (b.faction ? 2 : 0) + ((b.plotThreads || []).length ? 1 : 0);
      if (aScore !== bScore) return bScore - aScore;
      return (b.lastActive || 0) - (a.lastActive || 0);
    });
    const visibleNpcs = activeNpcs.slice(0, 8);
    const npcsText = visibleNpcs.length
      ? visibleNpcs.map(n => {
          const parts = [];
          if (n.faction) parts.push(n.faction + '·' + (n.role || '成员'));
          else if (n.role) parts.push(n.role);
          let s = `- ${n.name}`;
          if (parts.length) s += `（${parts.join('')}）`;
          if (n.location) s += `在${n.location}`;
          if (n.goal) s += `，目标是${n.goal}`;
          if (n.portrait) s += `。${n.portrait}`;
          const activeThreads = (n.plotThreads || []).filter(t => t.status === 'active');
          if (activeThreads.length) s += `线索：${activeThreads.map(t => t.thread).join('、')}`;
          return s;
        }).join('\n')
      : '无';

    // 本轮刚死亡的 NPC 讣告
    const recentlyDead = (worldState.npcs || []).filter(n =>
      n.status === 'dead' && n.deathInfo && n.deathInfo.round === worldState.round
    );
    const deathNotice = recentlyDead.length
      ? '\n' + recentlyDead.map(n => `⚰️ ${n.name}${n.deathInfo.cause ? '（' + n.deathInfo.cause + '）' : ''}${n.deathInfo.impact ? '——' + n.deathInfo.impact : ''}`).join('\n')
      : '';
    ...
    // 成就摘要
    if (worldState.achievements && worldState.achievements.unlocked) {
      const recent = Object.entries(worldState.achievements.unlocked).slice(-3);
      if (recent.length) {
        newSystemParts.push(`成就：已解锁${worldState.achievements.totalUnlocked || 0}个，最近：${recent.map(([k,v]) => v.title || k).join('、')}`);
      }
    }

    // 战斗摘要
    if (worldState.combat && worldState.combat.totalBattles > 0) {
      const ct = worldState.combat;
      const rate = ct.totalBattles > 0 ? (ct.wins / ct.totalBattles * 100).toFixed(0) : '0';
      newSystemParts.push(`战斗：${ct.totalBattles}战${ct.wins}胜，胜率${rate}%，击杀${ct.totalKills || 0}人`);
    }
    ...
    // 情感摘要
    if (worldState.emotionMap && Object.keys(worldState.emotionMap).length > 0) {
      const emoEntries = Object.entries(worldState.emotionMap).slice(0, 5);
      const emoText = emoEntries.map(([n, e]) => `${n}:${e.emotion || e.attitude || '?'}`).join('、');
      if (emoText) newSystemParts.push(`情感：${emoText}`);
    }

    // 全局线索
    if (worldState.globalPlotThreads) {
      const activeThreads = worldState.globalPlotThreads.filter(t => t.status === 'active').slice(0, 3);
      if (activeThreads.length) {
        newSystemParts.push(`线索：${activeThreads.map(t => `${t.title}(${t.progress}%)`).join('、')}`);
      }
    }
    ...
    // 连击
    if (worldState.combo > 0) {
      const badge = core.getComboBadge ? core.getComboBadge(worldState.combo) : '';
      newSystemParts.push(`连击：${worldState.combo}连${badge ? ' ' + badge : ''}`);
    }

    const newSystemsText = newSystemParts.length ? '\n' + newSystemParts.join('\n') : '';
...
区域事件：${riText}
黑盒：${blackboxText}
NPC：${npcsText}${deathNotice}${newSystemsText}
